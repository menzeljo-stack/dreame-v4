...
class DreameVacuum extends IPSModule
{
    ...
    public function Create()
    {
        ...
        $this->RegisterTimer('UpdateTimer', 0, 'DREAME_Update($_IPS["TARGET"]);');
    }
    ...
    private function EnsureHostLoaded()
    {
        ...
        // fetch now
        $info = $this->ApiCall('dreame-user-iot/iotuserbind/device/info', array('did' => $this->GetDID()));
        if (is_array($info) && isset($info['code']) && (int)$info['code'] === 0 && isset($info['data']) && is_array($info['data']) && isset($info['data']['bindDomain'])) {
            $host = strval($info['data']['bindDomain']);
    ...
    private function SendCommand($method, $params)
    {
        ...
        if ($host === '') throw new Exception('Host/Cluster unbekannt. Setze "Host" aus HA oder führe einmal Update() aus.');

        $parts = explode('.', $host);
        $cluster = '';
        if (count($parts) > 0) {
            $cluster = '-' . $parts[0]; // "-10000"
        }
