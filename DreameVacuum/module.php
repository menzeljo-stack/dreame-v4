...
class DreameVacuum extends IPSModule
{
    ...
    public function Create()
    {
        ...
        $this->RegisterPropertyString('MopPadKey', ''); // installed/not installed
        $this->RegisterPropertyString('RelocationStatusKey', '');
        $this->RegisterPropertyString('SelfWashBaseStatusKey', '');
        $this->RegisterPropertyString('StateKey', '');
        $this->RegisterPropertyString('StatusKey', '');
        $this->RegisterPropertyString('StreamStatusKey', '');
        $this->RegisterPropertyString('TaskStatusKey', '');
        $this->RegisterPropertyString('TaskTypeKey', '');
        ...
        try {
            ...
            foreach ($result as $item) {
                ...
                if ($relocKeyStr !== '' && $key === $relocKeyStr) {
                    ...
                    $this->SetVarString('RelocationStatus', $mapped !== null ? $mapped : (is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val)));
                }
                if ($selfWashKeyStr !== '' && $key === $selfWashKeyStr) {
                    $mapped = $this->MapByJson($val, $selfWashMap);
                    $this->SetVarString('SelfWashBaseStatus', $mapped !== null ? $mapped : (is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val)));
                }
                if ($stateKeyStr !== '' && $key === $stateKeyStr) {
                    $this->SetVarString('State', is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val));
                }
                if ($statusKeyStr !== '' && $key === $statusKeyStr) {
                    $this->SetVarString('Status2', is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val));
                }
                if ($streamKeyStr !== '' && $key === $streamKeyStr) {
                    $mapped = $this->MapByJson($val, $streamStatusMap);
                    $this->SetVarString('StreamStatus', $mapped !== null ? $mapped : (is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val)));
                }
                if ($taskStatusKeyStr !== '' && $key === $taskStatusKeyStr) {
                    $mapped = $this->MapByJson($val, $taskStatusMap);
                    $this->SetVarString('TaskStatus', $mapped !== null ? $mapped : (is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val)));
                }
                if ($taskTypeKeyStr !== '' && $key === $taskTypeKeyStr) {
                    $this->SetVarString('TaskType', is_array($val) ? json_encode($val, JSON_UNESCAPED_UNICODE) : strval($val));
                }
...
$this->EnsureIntProfile('DRMV.ChargingState', array(
    ...
    private function EnsureVariables()
    {
        ...
        $this->MaintainVariable('RelocationStatus', 'Relocation Status', VARIABLETYPE_STRING, '~TextBox', 171, true);
        $this->MaintainVariable('SelfWashBaseStatus', 'Self-Wash Base Status', VARIABLETYPE_STRING, '~TextBox', 172, true);
        $this->MaintainVariable('State', 'State', VARIABLETYPE_STRING, '~TextBox', 173, true);
        $this->MaintainVariable('Status2', 'Status (raw)', VARIABLETYPE_STRING, '~TextBox', 174, true);
        $this->MaintainVariable('StreamStatus', 'Stream Status', VARIABLETYPE_STRING, '~TextBox', 175, true);
        $this->MaintainVariable('TaskStatus', 'Task Status', VARIABLETYPE_STRING, '~TextBox', 176, true);
        $this->MaintainVariable('TaskType', 'Task Type', VARIABLETYPE_STRING, '~TextBox', 177, true);
